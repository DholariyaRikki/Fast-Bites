...
const API_END_POINT = "http://localhost:3350/api/menu";
...
type MenuState = {
    ...
    updateMenuAvailability: (menuId: string, isAvailable: boolean) => Promise<void>;
}
...
export const useMenuStore = create<MenuState>()(persist((set) => ({
    ...
    deleteMenu: async (menuId: string) => {
        try {
            ...
            if (response.data.success) {
                toast.success(response.data.message);
                set({ loading: false });
                // Remove menu from restaurant state
    ...
    updateMenuAvailability: async (menuId: string, isAvailable: boolean) => {
            try {
                set({ loading: true });
                const response = await axios.put(`${API_END_POINT}/${menuId}/availability`, { isAvailable });
                if (response.data.success) {
                    toast.success(response.data.message);
                    set({ loading: false });
                    // Update restaurant menu with new availability
                    useRestaurantStore.getState().updateMenuToRestaurant(response.data.menu);
                }
            } catch (error: any) {
                toast.error(error.response.data.message);
